...
import { Shield, Eye, EyeOff, Bell, Lock, UserX, Smartphone, ChevronLeft, Moon, Sun, Palette } from "lucide-react";
import { useApp } from "../context/AppContext";

interface Toggle { label: string; desc?: string; value: boolean; }

function ToggleRow({ label, desc, value, onChange }: Toggle & { onChange: () => void }) {
  return (
...
export default function SettingsPage() {
  ...
  const [privacy, setPrivacy] = useState({
    showPhone: false, showAge: true, showMarital: false,
    hideLastSeen: false, twoFactor: false,
  });
  const [notifs, setNotifs] = useState({
    messages: true, friendRequests: true, comments: true, likes: true, mentions: true,
  });
  ...
  const SECTIONS = [
    ...
    { id: "appearance", icon: <Palette size={20} />, label: "المظهر", desc: "الوضع الليلي والألوان" },
      ...
      <div className="page-container">
        ...
        {section === "privacy" && (
          <div className="settings-section">
            ...
            <ToggleRow label="إظهار رقم الهاتف" desc="يستطيع الآخرون رؤية رقمك" value={privacy.showPhone} onChange={() => setPrivacy(p => ({ ...p, showPhone: !p.showPhone }))} />
            ...
            <ToggleRow label="إخفاء آخر ظهور" desc="لن يرى أحد متى كنت نشطاً" value={privacy.hideLastSeen} onChange={() => setPrivacy(p => ({ ...p, hideLastSeen: !p.hideLastSeen }))} />
        ...
        {section === "appearance" && (
          <div className="settings-section">
            ...
            <div className="settings-row">
              <div>
                <div className="settings-row-label">{darkMode ? "الوضع الليلي" : "الوضع النهاري"}</div>
                <div className="settings-row-desc">تبديل بين الوضعين</div>
              </div>
